...
void LinearModel::standardise() {
	// standardize phenotype from previously calculated mean and 
	// variance stored in member variables meanY and varY
	double sdY = sqrt(varY);
	for(int i = 0; i < nind; i++) {
		Y[i] = (Y[i] - meanY) / sdY;
	}

	// calculate the mean and variance for all variables in the model
	// temporary vectors for the mean and standard deviation of each variable	
	vector_t mX(np, 0);
	vector_t sdX(np, 0);
...
void LinearModel::fitLM() {
	...
	// PLINK comment: Perform "svdfit(C,Y,sig,b,u,v,w,chisq,function)"
	// ----- bcw 4/29/13 
	// From: "Numerical Recipes: The Art of Scientific Computing" (Fortran)
	// svdfit  on p. 518.
	...
	*/	
	...
	// Uses "back substitution" to solve the set of simultaneous linear equations 
	// Ax = b, given the u, w, and v arrays created by the SVD procedure from the
	// matrix a.
	svbksb(u, w, v, b, coef);
...
void LinearModel::fitUnivariateLM() {
	...
	S[1][1] = (y_var / x_var - (y_x_covar * y_x_covar) / 
					(x_var * x_var)) / (nind - 2);
}
