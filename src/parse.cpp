...
void setOptions(CArgs & a) {
  ...
  if(a.find("--test-missing")) {
    par::test_missing = true;
  }

  if(a.find("--test-mishap")) {
  ...
  if(a.find("--hap-pp")) {
    par::hap_post_prob = a.value_double("--hap-pp");
  }
  if(a.find("--hap-miss")) {
    par::hap_missing_geno = a.value_double("--hap-miss");
  }
  ...
  if(a.find("--regain-matrix-threshold-value")) {
    par::regainMatrixTransform = "threshold";
    par::regainMatrixThreshold = true;
    par::regainMatrixThresholdValue =
            a.value_double("--regain-matrix-threshold-value");
  }

  if(a.find("--regain-matrix-transform")) {
    par::regainMatrixTransform = 
            a.value("--regain-matrix-transform");
  }

  if(a.find("--regain-matrix-format")) {
  ...
  if(a.find("--distance-matrix")) {
    par::distance_matrix = par::matrix = true;
  }
  ...
  if(a.find("--help") || a.find("-h")) {
    ...
    cout << "inbix --file {fileroot}            Specify .ped and .map files \n"
            ...
            << "\n"
            << "      --rank-by {ranker}                          centrality(_power|_gauss) | regression(b|s|p)\n"
            ...
            << "      --permute-gain-threshold {threshold}        Permute GAIN+SNPrank significance threshold\n"
            << "\n"
