...
void Plink::calcEpistasis() {
  ...
  // added by Bill White - 5/26/11
  ofstream BETAS;
  string beta_f = par::output_file_name;
  beta_f += ".epi.betas";
  BETAS.open(beta_f.c_str(), ios::out);
  printLOG("Writing epistasis pairwise beta values to [ " + beta_f + " ] \n");
  BETAS.precision(6);
  ...
  // TO DO: added OpenMP parallelization of at least this outer loop
    ...
    if(sA[e1]) {
      ...
      for(int e2 = 0; e2 < nl_all; e2++) {
        ...
        if(!par::fast_epistasis) {
          ...
          // added by bcw 5/27/11
          BETAS << locus[e1]->name << "\t" << locus[e2]->name
                  << "\t" << b[0]
                  << "\t" << b[1]
                  << "\t" << b[2]
                  << "\t" << b[3]
