...
#pragma omp parallel for schedule(dynamic, 1) private(i, j)
    ...
    if((par::ecPrivacyTrainFile == "") || 
      ...
      if(!P.sample.size() > 3) {
        error("Privacy EC requires either 1) train, holdout and test files; or "
              "2) files loaded with --bfile/--file and/or --numeric-file.\n");
      } else {
        // --------------------------------------------------------------------
        // SPLIT PLINK loaded data set and run EvaporativeCoolingPrivacy 
        P.SNP2Ind();
        P.printLOG("Splitting and loading PLINK internal data.\n");
        P.printLOG("\n");
        // split the PLINK internal data into three Datasets
        PlinkInternalsDataset plinkInternalsDataset(&P);
        ...
        vector<uint> idxSetCtrls = classIdx[2];
        ...
        if(!trainDs->LoadOtherDatasetInstances(&plinkInternalsDataset, idxSetTrain)) {
          error("Training Dataset initialization failed\n");
        }
        if(!holdoutDs->LoadOtherDatasetInstances(&plinkInternalsDataset, idxSetHoldout)) {
          error("Holdout Dataset initialization failed\n");
        }
        if(!testDs->LoadOtherDatasetInstances(&plinkInternalsDataset, idxSetTest)) {
          error("Test Dataset initialization failed\n");
        }
      }
    } else {
      // ------------------------------------------------------------------------
      // run EvaporativeCoolingPrivacy here with 
      // data sets from files generated by other programs
