...
bool Plink::outputArffFile(string arffFilename) {
  ...
  ARFF << "% " << arffFilename << " written by inbix: " << tdstamp;
  ARFF << "%" << endl;
  ARFF << "@RELATION " << par::output_file_name << endl << endl;

  // write attribute section
  // write SNP attributes
  for(int i=0; i < nl_all; ++i) {
    ARFF << "@ATTRIBUTE " << locus[i]->name << " {0,1,2}" << endl;
  }
  ...
  if(par::bt) {
    ARFF << "@ATTRIBUTE CLASS {0,1}" << endl;
  }
  else {
    ARFF << "@ATTRIBUTE CLASS numeric" << endl;
  }
  ...
  for (int i = 0; i < sample.size(); i++) {
    ...
    for(int j=0; j < nl_all; ++j) {
      if(seenDataForLine) {
        ARFF << "," << getSimpleSNPValue(sample[i], j);
      }
      else {
        ARFF << getSimpleSNPValue(sample[i], j);
...
int Plink::getSimpleSNPValue(Individual* person, int snp) {
